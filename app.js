(function () {
  ...
  // 관람차(레벨7) 전용 — "그림 전체가 360도 도는데 지지대까지 같이 돌면 이상하다"는 피드백으로,
  // <image href>를 통째로 쓰는 대신 ferriswheel.svg의 원본 path를 그대로 가져와 두 그룹으로
  // 나눠 인라인으로 그린다: 지지대(다리+받침)는 안 움직이는 그룹, 바퀴(테두리/스포크/칸 8개/
  // 중심축)는 도는 그룹. 좌표계 변환은 다른 인라인 이모지(시계)와 동일.
  const FERRISWHEEL_STATIC =
    '<path fill="#8899A6" d="M27.279 34.429l-8.376-18.344c-.021-.046-.058-.076-.085-.117-.038-.059-.072-.118-.123-.169-.051-.051-.11-.085-.17-.123-.041-.027-.071-.064-.117-.085-.012-.005-.024-.003-.035-.008-.064-.027-.132-.034-.201-.047-.061-.011-.121-.03-.182-.03-.054 0-.106.018-.161.027-.075.013-.149.022-.219.051-.011.005-.023.003-.034.008-.044.02-.074.057-.114.082-.061.039-.121.074-.173.125-.051.051-.086.111-.124.171-.026.04-.063.07-.083.115L8.709 34.429c-.229.502-.008 1.096.494 1.325.135.061.276.09.415.09.379 0 .742-.217.91-.585l7.465-16.351 7.465 16.351c.168.368.531.585.911.585.139 0 .279-.029.414-.091.504-.229.725-.822.496-1.324z"/>' +
    '<path fill="#553788" d="M12.9 36H7.079c-.552 0-1-.447-1-1s.448-1 1-1H12.9c.552 0 1 .447 1 1s-.448 1-1 1zm15.999 0h-5.82c-.553 0-1-.447-1-1s.447-1 1-1h5.82c.553 0 1 .447 1 1s-.447 1-1 1z"/>';
  const FERRISWHEEL_SPIN =
    ...
    '<path fill="#744EAA" d="M21.284 29.819c0 1.535-1.244 2.779-2.779 2.779h-1.112c-1.535 0-2.779-1.244-2.779-2.779s1.244-2.779 2.779-2.779h1.112c1.534 0 2.779 1.244 2.779 2.779z"/>' +
    '<path fill="#CBB7EA" d="M21.227 29.263c-.257-1.269-1.378-2.224-2.723-2.224h-1.112c-1.345 0-2.466.954-2.724 2.224h6.559z"/>' +
    ...
    '<path fill="#C6E5B3" d="M10.96 25.971c-.257-1.269-1.378-2.224-2.723-2.224H7.126c-1.345 0-2.466.954-2.724 2.224h6.558z"/>' +
    ...
    '<path fill="#BBDDF5" d="M34.921 17.077c-.257-1.269-1.378-2.224-2.723-2.224h-1.112c-1.344 0-2.466.954-2.723 2.224h6.558z"/>' +
    ...
    '<path fill="#C6E5B3" d="M31.588 7.331c-.257-1.269-1.378-2.224-2.723-2.224h-1.112c-1.344 0-2.466.954-2.723 2.224h6.558z"/>' +
    '<path fill="#BE1931" d="M31.644 26.527c0 1.535-1.244 2.779-2.779 2.779h-1.112c-1.535 0-2.779-1.244-2.779-2.779s1.244-2.779 2.779-2.779h1.112c1.535 0 2.779 1.244 2.779 2.779z"/>' +
    '<path fill="#F4ABBA" d="M31.588 25.971c-.257-1.269-1.378-2.224-2.723-2.224h-1.112c-1.344 0-2.466.954-2.723 2.224h6.558z"/>' +
    ...
    '<path fill="#F4ABBA" d="M10.96 7.331c-.257-1.269-1.378-2.224-2.723-2.224H7.126c-1.345 0-2.466.954-2.724 2.224h6.558z"/>' +
    ...
    '<path fill="#CBB7EA" d="M21.227 2.32C20.97 1.051 19.849.096 18.504.096h-1.112c-1.345 0-2.466.954-2.724 2.224h6.559z"/>' +
    ...
    '<path fill="#BBDDF5" d="M7.625 17.077c-.257-1.269-1.378-2.224-2.723-2.224H3.79c-1.345 0-2.466.954-2.724 2.224h6.559z"/>' +
  ...
  const FERRISWHEEL_INLINE =
    '<g class="reward-emoji-img" transform="translate(8.333,8.333) scale(2.3148)">' +
    '<g>' + FERRISWHEEL_STATIC + '</g>' +
    '<g class="reward-wheel-spin">' + FERRISWHEEL_SPIN + '</g>' +
    '</g>';
  ...
  function buildRewardSvg(art, cols, rows) {
    ...
    } else if (!art.flyDirection) { // 위 특수 연출이 없는 나머지(관람차는 바퀴 자체가 회전, 여기선 색종이만 추가)는 기본 색종이
      extra += confettiGroup([[38, 40, -10], [50, 32, 15], [62, 40, -20], [44, 48, 20], [56, 46, -15], [50, 54, 10]]);
    }
    const base = art.effect === 'wheel-spin'
      ? FERRISWHEEL_INLINE
  ...
  const LEVEL_REWARD_ART = {
    ...
    7: { emoji: 'ferriswheel', effect: 'wheel-spin' },  // 놀이 — 완성되면 바퀴만 실제로 회전(지지대 고정)
